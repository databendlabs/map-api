...
fn test_from_tuple() -> anyhow::Result<()> {
    let m = SeqMarked::from((1, 2u64, Some(3u64)));

    assert_eq!(m, SeqMarked::new_with_meta(1, 2, Some(3u64)));
...
fn test_impl_trait_seq_value() -> anyhow::Result<()> {
    let m = SeqMarked::<u32, u64>::new_with_meta(1, 2, None);
    assert_eq!(m.seq(), 1);
    assert_eq!(m.value(), Some(&2));
    assert_eq!(m.meta(), None);

    let m = SeqMarked::new_with_meta(1, 2, Some(3u32));
    assert_eq!(m.seq(), 1);
    assert_eq!(m.value(), Some(&2));
    assert_eq!(m.meta(), Some(&3u32));

    let m: SeqMarked<u64> = SeqMarked::new_tombstone(1);
...
fn test_empty() -> anyhow::Result<()> {
    let m = SeqMarked::<u64>::empty();
    assert_eq!(m, SeqMarked::TombStone { internal_seq: 0 });
...
fn test_max() -> anyhow::Result<()> {
    let m1 = SeqMarked::new_with_meta(1, 2, None);
    let m2 = SeqMarked::new_with_meta(3, 2, None);
    let m3: SeqMarked<u32, u64> = SeqMarked::new_tombstone(2);
...
fn test_from_marked_for_option_seqv() -> anyhow::Result<()> {
    let m = SeqMarked::new_with_meta(1, 2, None);
    ...
    let m: SeqMarked<u32, u64> = SeqMarked::new_tombstone(1);
